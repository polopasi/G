...
void Reflection::onPluginLoad()
{
    ...
    vsMirror->compileSourceFile(g.getPluginPath()+"/../reflection/mirror.vert");

    fsMirror = new QOpenGLShader(QOpenGLShader::Fragment, this);
    fsMirror->compileSourceFile(g.getPluginPath()+"/../reflection/mirror.frag");
    ...
    g.glGenTextures( 1, &textureOfReflection);
    g.glBindTexture(GL_TEXTURE_2D, textureOfReflection);
...
void drawRect(GLWidget &g)
{
    ...
    if (!created)
    {
        ...
        float coords[] = { -2*soil.x(), soil.y(), -2*soil.z(), 
                            2*soil.x(), soil.y(), -2*soil.z(), 
                           -2*soil.x(), soil.y(),  2*soil.z(),
                            2*soil.x(), soil.y(),  2*soil.z()}; 
...
bool Reflection::paintGL()
{
    ...
    program->setUniformValue("soilY", scene()->boundingBox().min().y());  
